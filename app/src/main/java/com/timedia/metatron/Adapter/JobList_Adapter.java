...
public class JobList_Adapter extends RecyclerView.Adapter<JobList_Adapter.ViewHolder> {
    ...
    public JobList_Adapter(Context aContext, ArrayList<JobList_Response.JobsList> aJobsList, Jobs_Onclick aJobs_Onclick) {
    ...
    @NonNull
    @Override
    public JobList_Adapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        return new JobList_Adapter.ViewHolder(LayoutInflater.from(parent.getContext())
                .inflate(R.layout.jobview, parent, false));
    }
    ...
    @SuppressLint("NewApi")
    @Override
    public void onBindViewHolder(@NonNull JobList_Adapter.ViewHolder holder, int position) {
        ...
        if (!TextUtils.isEmpty(mJobsList.get(position).getJob_iscompleted())) {
            if (mJobsList.get(position).getJob_iscompleted().equalsIgnoreCase("1")) {
    ...
    public class ViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        ...
        private TextView mCustomerName, mAddress, mMobile, mEmail, mCost, mDescription,mJobinvoice;
