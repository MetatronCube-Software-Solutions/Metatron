...
public class Jobs_Fragment extends Fragment implements Jobs_Onclick {
    ...
    private void Load_JobListApi() {
        ...
        Call<JobList_Response> aJobsApi = Utils_Functions.Get_API_Services().ListJobs("1", PreferenceDetails.GetUserid());
                ...
                if (response.code() == 200) {
                    ...
                    if (mListJobArray != null) {
                        if (mListJobArray.size() > 0) {
                            mJobs_Recyclerview.setAdapter(new JobList_Adapter(getActivity(), mListJobArray, Jobs_Fragment.this));
                        } else {
