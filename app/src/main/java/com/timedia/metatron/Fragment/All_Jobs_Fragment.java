...
import com.timedia.metatron.Adapter.All_Job_Adapter;
...
public class All_Jobs_Fragment extends Fragment implements View.OnClickListener, Jobs_Onclick {
    ...
    private void Load_JobListApi() {
        ...
        Call<JobList_Response> aJobsApi = Utils_Functions.Get_API_Services().ListAllJobs(PreferenceDetails.GetUserid());
                ...
                if (response.code() == 200) {
                    ...
                    if (mListJobArray != null) {
                        if (mListJobArray.size() > 0) {
                            mJobs_Recyclerview.setAdapter(new All_Job_Adapter(getActivity(), mListJobArray, All_Jobs_Fragment.this));
                        } else {
    ...
    public void onJobs_OnclickCallback(final int position, int type) {
        if (type == 0) {
            ...
            if (!mListJobArray.get(position).getJob_iscompleted().equalsIgnoreCase("1")) {
